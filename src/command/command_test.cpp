...
SCENARIO("/read command execution") {
  GIVEN("a file that exists") {
    ...
    WHEN("/read is executed") {
      ...
      THEN("confirmation is printed") { CHECK(out.str().find("loaded") != std::string::npos); }
    }
  }
  ...
  GIVEN("a file that does not exist") {
    ...
    WHEN("/read is executed") {
      ...
      THEN("it fails") { CHECK_FALSE(ok); }
      THEN("history is unchanged") { CHECK(history.empty()); }
      THEN("error is printed") { CHECK(out.str().find("Error") != std::string::npos); }
    }
  }
}
