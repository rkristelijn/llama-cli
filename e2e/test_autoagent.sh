...
export HOME="/tmp/test-autoagent-$$"
mkdir -p "$HOME"
# Use CLI flag --provider=mock to override .env (env var loses to .env in load order)
MOCK_ARGS=(--repl --provider=mock)
...
OUTPUT=$(echo -e "@explore find TODO comments\nexit" | "$BINARY" "${MOCK_ARGS[@]}" 2>&1) || die "@explore failed"
...
OUTPUT=$(echo -e "/agent plan\nhello\nexit" | LLAMA_CLI_MOCK_RESPONSE="$MOCK_RESP" "$BINARY" "${MOCK_ARGS[@]}" 2>&1) || die "plan agent failed"
...
OUTPUT=$(echo -e "@nonexistent do something\nexit" | "$BINARY" "${MOCK_ARGS[@]}" 2>&1) || die "@nonexistent failed"
